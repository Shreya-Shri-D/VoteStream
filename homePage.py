...
def Home(root, frame1, frame2):
    ...
    frame1.config(bg="#D4E4F7") 
    frame2.config(bg="#D4E4F7") 
    root.config(bg="#D4E4F7")
   
    
    Button(frame2, text="Home", command = lambda: Home(root, frame1, frame2)).grid(row=0,column=0)
    Label(frame2, text="                                                                         ",bg="#D4E4F7").grid(row = 0,column = 1)
    Label(frame2, text="                                                                         ",bg="#D4E4F7").grid(row = 0,column = 2)
    Label(frame2, text="         ",bg="#D4E4F7").grid(row = 1,column = 1)
    
 
    frame2.pack(side=TOP)
    root.title("Home")

    Label(frame1, text="Home",bg="#D4E4F7", font=('Helvetica', 25, 'bold')).grid(row = 0, column = 1, rowspan=1)
