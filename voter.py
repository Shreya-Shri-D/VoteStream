...
def failed_return(root,frame1,client_socket,message):
    for widget in frame1.winfo_children():
        widget.destroy()
    message = message + "... \nTry again..."
    Label(frame1, text=message, font=('Helvetica', 12, 'bold'),bg="#D4E4F7").grid(row = 1, column = 1)
    client_socket.close()
...
def voterLogin(root,frame1):
    ...
    Label(frame1, text="Voter Login", bg="#D4E4F7",font=('Helvetica', 18, 'bold')).grid(row = 0, column = 2, rowspan=1)
    Label(frame1, text="",bg="#D4E4F7").grid(row = 1,column = 0)
    Label(frame1, text="Voter ID:      ",bg="#D4E4F7", anchor="e", justify=LEFT).grid(row = 2,column = 0)
    Label(frame1, text="Password:   ", bg="#D4E4F7",anchor="e", justify=LEFT).grid(row = 3,column = 0)
